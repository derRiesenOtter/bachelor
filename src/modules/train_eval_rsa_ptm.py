    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for i, vdata in enumerate(val_loader):
                vinputs, vrsavalues, vptm, vlabels = vdata
                vinputs, vrsavalues, vptm, vlabels = (
                    vinputs.to(device),
                    vrsavalues.to(device),
                    vptm.to(device),
                    vlabels.to(device),
                )
                voutputs = model(vinputs, vrsavalues, vptm)
...
def train_one_epoch(model, train_loader, device, loss_fn, optimizer):
    ...
    for i, data in enumerate(train_loader):
        inputs, rsa_input, ptm, labels = data
        inputs, rsa_input, ptm, labels = (
            inputs.to(device),
            rsa_input.to(device),
            ptm.to(device),
            labels.to(device),
        ...
        optimizer.zero_grad()
        outputs = model(inputs, rsa_input, ptm)
