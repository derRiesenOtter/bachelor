    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for i, vdata in enumerate(val_loader):
                vinputs, vrsavalues, vlabels = vdata
                vinputs, vrsavalues, vlabels = (
                    vinputs.to(device),
                    vrsavalues.to(device),
                    vlabels.to(device),
                )
                voutputs = model(vinputs, vrsavalues)
...
def train_one_epoch(model, train_loader, device, loss_fn, optimizer):
    ...
    for i, data in enumerate(train_loader):
        inputs, rsa_input, labels = data
        inputs, rsa_input, labels = (
            inputs.to(device),
            rsa_input.to(device),
            labels.to(device),
        )
        ...
        optimizer.zero_grad()
        outputs = model(inputs, rsa_input)
