...
def annot_PTMs(
    protein_names: list,
    source: list = ["uniprot"],
    return_failed_attempts: bool = False,
    ...
    for index, prot in enumerate(protein_names):
        ...
        for ptm in payload["features"]:
            ...
            try:
                ...
                temp_dict2 = dict(
                    (k, ptm[k]) for k in ("type", "description")
                )  # 'begin', 'end', 'molecule'
...
def annot_IDR(
    ...
    for index, prot in enumerate(protein_names):
        ...
        temp_dict2 = {
            k: reg_to_list(payload.get(k, {}).get("regions"), len(payload["sequence"]))
            for k in (
        ...
        temp_dict2["prediction-disorder-alphafold-score"] = payload.get(
            "prediction-disorder-alphafold", {}
        ).get("scores")
...
def annot_domains(
    ...
    result_list, failed_list, failed_list_e = (
        ...
        [],
        [],
    )  # lists to store successful and failed requests
    ...
    for index, prot in enumerate(protein_names):
        ...
        if prot in cache:
            ...
            # Handle any exceptions that may occur during the API request
                ...
                failed_list.append(
                    prot
                )  # add the protein to the list of failed requests
                failed_list_e.append(e)
                continue
        ...
        for el in payload["results"]:
            ...
            if el["metadata"]["type"] == "domain":
                temp_dict["protein_name"] = (
                    prot  # add the protein name to the dictionary
                )
                temp_dict["databases"] = list(
                    el["metadata"]["member_databases"].keys()
                )  # add list of databases to the dictionary
                temp_dict2 = dict(
                    (k, el["metadata"][k]) for k in ("accession", "name")
                )  # add the accession and name to the dictionary
                temp_dict2[
                    "GO_identifiers"
                ] = []  # create an empty list for GO identifiers
                temp_dict2["GO_names"] = []  # create an empty list for GO names
                if el[
                    "metadata"
                ][
                    "go_terms"
                ]:  # if there are GO terms, extract the identifier and name and add them to the corresponding lists
                    for id in el["metadata"]["go_terms"]:
                        temp_dict2["GO_identifiers"].append(id["identifier"])
                        temp_dict2["GO_names"].append(id["name"])
                temp_dict3 = el["proteins"][0]["entry_protein_locations"][0][
                    "fragments"
                ][0]  # extract the domain location information
                result_list.append(
                    {**temp_dict, **temp_dict2, **temp_dict3}
                )  # combine the 3 dictionaries and add to the result list
...
def annot_GO(
    ...
    list_of_GO_terms_withRNAbinding = (
        open(r"/mnt/d/phd/scripts/raw_data/GO_terms/GO_terms_RNAbinding.txt", "r")
        .read()
        .split("\n")[:-1]
    )
    list_of_GO_terms_withNAbinding = (
        open(r"/mnt/d/phd/scripts/raw_data/GO_terms/GO_terms_NAbinding.txt", "r")
        .read()
        .split("\n")[:-1]
    )
    list_of_GO_terms_withDNAbinding = (
        open(r"/mnt/d/phd/scripts/raw_data/GO_terms/GO_terms_DNAbinding.txt", "r")
        .read()
        .split("\n")[:-1]
    )
