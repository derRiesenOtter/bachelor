...
train_df, val_df = train_test_split(
    df, test_size=0.2, stratify=df["ps_cd_label"], random_state=13
)

# Create DataLoaders that are
# responsible for feeding the data into the model
train_data_set = SequenceDataSet(train_df, "mapped_seq", "ps_cd_label")
train_loader = DataLoader(train_data_set, batch_size=32, shuffle=True)
val_data_set = SequenceDataSet(val_df, "mapped_seq", "ps_cd_label")
...
class_weights = compute_class_weight(
    ...
    classes=np.unique(df["ps_cd_label"]),
    y=df["ps_cd_label"],
)
