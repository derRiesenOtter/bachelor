...
with open("./data/intermediate_data/ppmclab.pkl", "rb") as f:
    df_ppmclab = pickle.load(f)

df_ppmclab.ps_cd_label

df["role"] = 0

uniprot_to_role = dict(zip(df_ppmclab["UniProt.Acc"], df_ppmclab["ps_cd_label"]))

df["role"] = df["UniprotEntry"].map(uniprot_to_role)
...
df = df.loc[(df["role"] == 2) | (df["ps_label"] == 0)]
