...
train_df.loc[(train_df["ps_label"] == 0), "ps_idr_label"] = 0
train_df.loc[
    (train_df["ps_label"] == 1) & (train_df["idr_protein"] == 0), "ps_idr_label"
] = 1
train_df.loc[
    (train_df["ps_label"] == 1) & (train_df["idr_protein"] == 1), "ps_idr_label"
] = 2
val_df.loc[val_df["ps_label"] == 0, "ps_idr_label"] = 0
val_df.loc[(val_df["ps_label"] == 1) & (val_df["idr_protein"] == 0), "ps_idr_label"] = 1
val_df.loc[(val_df["ps_label"] == 1) & (val_df["idr_protein"] == 1), "ps_idr_label"] = 2
...
train_data_set = SequenceDataSet(train_df, "mapped_seq", "ps_idr_label")
train_loader = DataLoader(train_data_set, batch_size=32, shuffle=True)
val_data_set = SequenceDataSet(val_df, "mapped_seq", "ps_idr_label")
...
class_weights = compute_class_weight(
    ...
    classes=np.unique(all_df["ps_idr_label"]),
    y=all_df["ps_idr_label"],
)
